'use client';

import { use, useEffect, useState } from 'react';
import Link from 'next/link';
import dynamic from 'next/dynamic';
import { useSession, useJoinSession, useLeaveSession } from '@/features/sessions/hooks/useSessions';
import { usePermissions } from '@/hooks/usePermissions';
import { useAuthStore } from '@/store/auth.store';
import { ParticipantList } from '@/features/sessions/components/ParticipantList';
import { SessionControls } from '@/features/sessions/components/SessionControls';
import { ClusterManager } from '@/features/clusters/components/ClusterManager';
import { StatusBadge } from '@/components/shared/StatusBadge';
import { PageContainer } from '@/components/layout/PageContainer';
import { LoadingSkeleton } from '@/components/feedback/LoadingSkeleton';
import { Button } from '@/components/ui/button';
import { Tabs, TabsContent, TabsList, TabsTrigger } from '@/components/ui/tabs';
import { ROUTES } from '@/constants/routes';
import { BarChart3, Activity, LogIn, LogOut, Download } from 'lucide-react';
import { ExportBoardDialog } from '@/features/ideas/components/ExportBoardDialog';

// Dynamic import for heavy tree component
const IdeaTree = dynamic(
  () => import('@/features/ideas/components/IdeaTree').then((m) => ({ default: m.IdeaTree })),
  { ssr: false, loading: () => <LoadingSkeleton variant="tree" /> }
);

export default function SessionPage({
  params,
}: {
  params: Promise<{ workspaceId: string; boardId: string; sessionId: string }>;
}) {
  const { workspaceId, boardId, sessionId } = use(params);
  const { data: session, isPending } = useSession(sessionId);
  const { canManageSessions, canManageClusters } = usePermissions(workspaceId);
  const { user } = useAuthStore();
  const joinSession = useJoinSession();
  const leaveSession = useLeaveSession();
  const [exportOpen, setExportOpen] = useState(false);

  if (isPending) return <LoadingSkeleton variant="page" />;
  if (!session) return null;

  return (
    <PageContainer
      title={session.title}
      description={session.description || 'Brainstorming session'}
      action={
        <div className="flex items-center gap-2">
          <StatusBadge status={session.status} />
          <SessionControls
            sessionId={sessionId}
            boardId={boardId}
            status={session.status}
            canManage={canManageSessions}
          />
          <Button variant="outline" size="sm" className="gap-1.5" onClick={() => setExportOpen(true)}>
            <Download className="h-3.5 w-3.5" /> Export
          </Button>
          <Link href={ROUTES.SESSION_ANALYTICS(workspaceId, boardId, sessionId)}>
            <Button variant="outline" size="sm" className="gap-1.5">
              <BarChart3 className="h-3.5 w-3.5" /> Analytics
            </Button>
          </Link>
          <Link href={ROUTES.SESSION_ACTIVITY(workspaceId, boardId, sessionId)}>
            <Button variant="outline" size="sm" className="gap-1.5">
              <Activity className="h-3.5 w-3.5" /> Activity
            </Button>
          </Link>
          {session.status !== 'ended' && (
            <Button
              variant="outline"
              size="sm"
              className="gap-1.5"
              onClick={() => joinSession.mutate(sessionId)}
              disabled={joinSession.isPending}
            >
              <LogIn className="h-3.5 w-3.5" /> Join
            </Button>
          )}
        </div>
      }
    >
      <div className="grid gap-6 lg:grid-cols-[1fr_300px] h-[calc(100vh-12rem)]">
        {/* Main content — Idea Tree */}
        <div id="idea-tree-export-root" className="border border-border/50 rounded-xl p-4 bg-card/30 overflow-hidden">
          <IdeaTree sessionId={sessionId} />
        </div>

        {/* Sidebar */}
        <div className="space-y-6 overflow-auto">
          <Tabs defaultValue="participants">
            <TabsList className="w-full">
              <TabsTrigger value="participants" className="flex-1">Participants</TabsTrigger>
              <TabsTrigger value="clusters" className="flex-1">Clusters</TabsTrigger>
            </TabsList>
            <TabsContent value="participants" className="mt-3">
              <ParticipantList
                sessionId={sessionId}
                participants={(session as any).participants ?? []}
                canManage={canManageSessions}
              />
            </TabsContent>
            <TabsContent value="clusters" className="mt-3">
              <ClusterManager sessionId={sessionId} canManage={canManageClusters} />
            </TabsContent>
          </Tabs>
        </div>
      </div>
      {exportOpen && (
        <ExportBoardDialog
          sessionId={sessionId}
          sessionTitle={session.title}
          open={exportOpen}
          onOpenChange={setExportOpen}
        />
      )}
    </PageContainer>
  );
}
