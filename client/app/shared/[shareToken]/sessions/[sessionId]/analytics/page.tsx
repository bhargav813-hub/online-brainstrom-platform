...
import { Users, Lightbulb, ThumbsUp, Activity, TrendingUp, BarChart3 } from 'lucide-react';
import { motion } from 'framer-motion';

export default function SharedSessionAnalyticsPage({
  params,
}: {
  ...
  const { data: sessionData, isPending: saLoading } = useQuery({
  ...
  const { data: voteAnalytics, isPending: vaLoading } = useQuery({
    queryKey: ['shared', 'votes', shareToken, sessionId],
    queryFn: () => sharedService.getVoteAnalytics(shareToken, sessionId),
    enabled: !!shareToken && !!sessionId,
  });
  ...
  const sessionAnalytics = sessionData.analytics;

  const stats = [
    {
      label: 'Active Participants',
      value: sessionAnalytics?.activeParticipants ?? 0,
      icon: Users,
  ...
  return (
    <PageContainer title="Session Analytics" description="Performance metrics and insights (Guest Access)">
      {/* Stats grid */}
              ...
              <div className="space-y-4">
                ...
                <div className="h-2 rounded-full bg-accent overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${
                        voteAnalytics.totalVotes > 0
                          ? (voteAnalytics.votesByType.upvotes / voteAnalytics.totalVotes) * 100
                          : 0
                      }%`,
                    }}
                  />
                ...
                <div className="h-2 rounded-full bg-accent overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${
                        voteAnalytics.totalVotes > 0
                          ? (voteAnalytics.votesByType.downvotes / voteAnalytics.totalVotes) * 100
                          : 0
                      }%`,
                    }}
                  />
